...
func generate_blocks():
	...
	for block_group in block_json["blocks"]:
		for base_block_name in block_group["base_blocks"]:
			var save_dir: String = base_block_name.to_pascal_case() + "/"
			dir.make_dir(save_dir)
			for block_data in block_group["block_types"]:
				...
				if "colour" in block_data:
					block.colour = Color(block_data["colour"])
					
				if "rotation" in block_data:
